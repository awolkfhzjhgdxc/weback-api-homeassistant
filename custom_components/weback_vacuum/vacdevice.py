...
class VacDevice(WebackWssCtrl):
    ...
    async def watch_state(self):
        ...
        try:
            await asyncio.gather(
                self.refresh_handler(self.name, self.sub_type),
                self.map_refresh_loop(),
            )
    ...
    async def load_maps(self):
        ...
        try:
            ...
            _LOGGER.warning("Failed to load map: %s", e)
        return False

    async def map_refresh_loop(self):
        """Periodically reload the reuse map and re-render the camera image.

        In HTTP-polling mode the cloud never pushes map_data (that only came
        over the blocked WSS), so the map would otherwise stay frozen at the
        image loaded on startup.
        """
        _LOGGER.debug("VacDevice: starting map refresh loop for= %s", self.name)
        while True:
            # Refresh faster while the robot is working so the path and
            # position on the camera stay reasonably current.
            interval = 30 if self.is_cleaning else 120
            await asyncio.sleep(interval)
            try:
                await self.load_maps()
            except Exception as map_excpt:
                _LOGGER.warning(
                    "VacDevice: map refresh failed (details=%s)",
                    map_excpt,
                )
    ...
    def is_available(self):
        """Boolean define if the cloud API is reachable.

        Marks the entity unavailable after 3 consecutive polling failures
        (~1.5 min) and recovers as soon as a poll succeeds.
        """
    ...
    def vacuum_or_mop(self) -> int:
        ...
        if "fan_status" in self.robot_status and "water_level" in self.robot_status:
            if (
